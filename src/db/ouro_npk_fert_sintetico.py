...
CSV_FILE = "src/db/ouro_npk_fert_sintetico.csv"
...
def upsert_data_to_db():
    ...
    with open(CSV_FILE, mode='r', encoding='utf-8') as file:
        ...
        for row in csv_reader:
            index, state, date, label, nutrient, value = row
    ...
    cursor.execute("SELECT * FROM public.tb_chart WHERE label = 'Fertilizantes Sintéticos' LIMIT 10")
    rows = cursor.fetchall()

    # Exibe os primeiros 10 resultados
    print("\nExibindo os primeiros 10 registros de 'Fertilizantes Sintéticos':")
    for row in rows:
        print(row)
