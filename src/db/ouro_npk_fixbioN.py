...
CSV_FILE = "src/db/ouro_npk_fixbioN.csv"
...
def upsert_data_to_db():
    ...
    cursor.execute("SELECT * FROM public.tb_chart WHERE label = 'Fixação biológica de N' LIMIT 10")
    rows = cursor.fetchall()

    # Exibe os primeiros 10 resultados
    print("\nExibindo os primeiros 10 registros de 'Fixação biológica de N':")
    for row in rows:
        print(row)
