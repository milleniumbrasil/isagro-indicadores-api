...
CSV_FILE = "src/db/ouro_npk_fert_organico_vinhaca.csv"
...
def upsert_data_to_db():
    ...
    cursor.execute("SELECT * FROM public.tb_chart WHERE label = 'Fertilizante orgânico vinhaça' LIMIT 10")
    rows = cursor.fetchall()

    # Exibe os primeiros 10 resultados
    print("\nExibindo os primeiros 10 registros de 'Fertilizante orgânico vinhaça':")
    for row in rows:
        print(row)
