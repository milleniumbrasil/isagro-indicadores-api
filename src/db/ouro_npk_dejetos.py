...
CSV_FILE = "src/db/ouro_npk_dejetos.csv"
...
def upsert_data_to_db():
    ...
    cursor.execute("SELECT * FROM public.tb_chart WHERE label LIKE 'Dejetos%' LIMIT 10")
    rows = cursor.fetchall()

    # Exibe os primeiros 10 resultados
    print("\nExibindo os primeiros 10 registros de 'Dejetos':")
    for row in rows:
        print(row)
