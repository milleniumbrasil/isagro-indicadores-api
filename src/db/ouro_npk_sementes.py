...
CSV_FILE = "src/db/ouro_npk_sementes.csv"
...
def upsert_data_to_db():
    ...
    cursor.execute("SELECT * FROM public.tb_chart WHERE label = 'Sementes' LIMIT 10")
    rows = cursor.fetchall()

    # Exibe os primeiros 10 resultados
    print("\nExibindo os primeiros 10 registros de 'Sementes':")
    for row in rows:
        print(row)
